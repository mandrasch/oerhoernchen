...
var generateList = function(list, selector) {
  $.each(list, function(index, object) {
    ...
    var html = '<div class="col-xs-6 col-sm-3 col-md-3 nopad text-center">' +
      ...
      '<img class="img-fluid" src="'+OERHOERNCHEN_ASSET_IMG_URL+'provider_logos/' + object.image + '" alt="'+ object.name+'" data-toggle="tooltip" data-placement="bottom" title="' + object.name + '" />' +
...
var generateListNotCompatible = function(list, selector) {
  $.each(list, function(index, object) {
    ...
    var html = '<div class="col-xs-6 col-sm-3 col-md-3 nopad text-center">' +
      ...
      '<img class="img-fluid" src="'+OERHOERNCHEN_ASSET_IMG_URL+'provider_logos/' + object.image + '" alt="' + object.name + '" title="' + object.name + '" data-toggle="tooltip" data-placement="bottom" title="' + object.name + '" />' +
      '</a>' +
      '</div>';
  ...
  $('select[name="license-filter"], select[name="web-search-license-filter"], select[name="media-search-license-filter"]').change(function() {
    ...
    switch (val) {
      case 'only-oer':
        ...
        $("span.license-filter-heading", explanation_header_div).html("Lizenzfilter: CC0, CC BY / SA");
        break;
      case 'nc':
        $('div[data-value="oer"]', explanation_div).show();
        $('div[data-value="nc"]', explanation_div).show();
        $("span.license-filter-heading", explanation_header_div).html("Lizenzfilter: CC0, CC BY / SA / NC");
        break;
      case 'nc-nd':
        $('div[data-value="oer"]', explanation_div).show();
        $('div[data-value="nc-nd"]', explanation_div).show();
        $("span.license-filter-heading", explanation_header_div).html("Lizenzfilter: CC0, CC BY / SA / NC / ND");
        break;
      case 'no-filter':
